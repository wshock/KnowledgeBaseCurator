...
export function SourcesModal({
  ...
  const handleLocalUpload = useCallback(async (file: File) => {
    if (file.type !== "application/pdf") {
      setPendingUploads((prev) => [
        ...prev,
        { file, status: "error", error: "Solo se aceptan PDFs" },
      ]);
      return;
    }
    ...
    const mockDoc: MockDocument = {
      ...
      scope: "local",
      chunks_indexed: Math.floor(file.size / 3000),
    };
    setLocalDocs((prev) => [...prev, mockDoc]);
    setSelectedLocal((prev) => new Set([...prev, mockId]));
    setPendingUploads((prev) =>
      prev.map((p) =>
        p.file === file ? { ...p, status: "success", docId: mockId } : p
      )
    );
  }, [token, chatId]);
  ...
  const handleConfirm = () => {
    onConfirm({
      globalIds: Array.from(selectedGlobal),
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden"
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Selecciona los archivos que SchoolAI usará como contexto
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex border-b border-gray-100 px-6">
          <button
            ...
            className={`flex items-center gap-1.5 text-xs font-semibold py-3 border-b-2 mr-5 transition-colors ${
              tab === "global"
          ...
          >
            ...
            {selectedGlobal.size > 0 && (
              <span className="ml-1 bg-[#1a2b4a] text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                {selectedGlobal.size}
              </span>
            )}
          </button>
          <button
            ...
            className={`flex items-center gap-1.5 text-xs font-semibold py-3 border-b-2 transition-colors ${
              tab === "local"
                ? "border-[#1a2b4a] text-[#1a2b4a]"
                : "border-transparent text-gray-400 hover:text-gray-600"
            }`}
            ...
            <div className="p-4 space-y-2">
              {globalDocs.length === 0 && (
                <div className="text-center py-10">
                  ...
                  <p className="text-xs text-gray-400">
                    No hay archivos globales aún.{" "}
                    <a href="/dashboard/subir-archivo" className="text-blue-600 underline">
                      Subir archivos
                    </a>
                  </p>
                </div>
              ...
              {globalDocs.map((doc) => {
                ...
                return (
                  <button
                    key={doc.id}
                    onClick={() => toggleGlobal(doc.id)}
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all text-left ${
                      isSelected
                        ? "border-[#1a2b4a] bg-[#f0f5ff]"
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-400 mt-0.5">
                        {formatSize(doc.size_bytes)} · {doc.chunks_indexed} fragmentos · {formatDate(doc.uploaded_at)}
                      </p>
                    </div>
                    <div className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-colors ${
                      isSelected ? "border-[#1a2b4a] bg-[#1a2b4a]" : "border-gray-300"
                    }`}>
                      {isSelected && (
          ...
          {tab === "local" && (
            <div className="p-4 space-y-3">
              ...
              <div
                ...
                className={`rounded-xl border-2 border-dashed p-5 flex flex-col items-center justify-center cursor-pointer transition-colors ${
                  isDragging ? "border-blue-400 bg-blue-50" : "border-gray-200 hover:border-blue-300 hover:bg-blue-50/40"
                }`}
              >
                <input
                  ref={fileInputRef}
              ...
              {pendingUploads.map((u, i) => (
                <div key={i} className="flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-100 bg-gray-50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-gray-400">{formatSize(u.file.size)}</p>
                  </div>
                  {u.status === "uploading" && (
                    <span className="flex items-center gap-1 text-[10px] text-blue-500 font-medium shrink-0">
                      <FiLoader className="h-3 w-3 animate-spin" /> Subiendo...
                    </span>
                  )}
                  {u.status === "success" && (
                  ...
                  {u.status === "error" && (
                    <span className="text-[10px] text-red-500 font-medium shrink-0">{u.error}</span>
                  )}
                </div>
              ))}
        ...
        <div className="border-t border-gray-100 px-6 py-4 flex items-center justify-between">
          <p className="text-xs text-gray-400">
            {totalSelected === 0
              ...
              : `${totalSelected} fuente${totalSelected !== 1 ? "s" : ""} seleccionada${totalSelected !== 1 ? "s" : ""}`}
          </p>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-4 py-2 text-xs font-semibold text-white bg-[#1a2b4a] hover:bg-blue-900 rounded-lg transition-colors"
            >
              Confirmar selección
            </button>
          </div>
