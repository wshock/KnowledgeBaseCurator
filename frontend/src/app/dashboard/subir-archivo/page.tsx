...
export default function SubirArchivoPage() {
  ...
  // Load existing global docs on mount
  useEffect(() => {
    const activeToken =
  ...
  const uploadFile = useCallback(
    async (file: File) => {
      ...
      try {
        const activeToken =
          token || (typeof window !== "undefined" ? localStorage.getItem("authToken") : "");
        if (!activeToken) throw new Error("No hay sesión activa");

        const data = await apiUploadGlobalDocument(activeToken, file);
        ...
        // Refresh the global docs list after a successful upload
        await reloadGlobalDocs();
      } catch (err) {
        const message = err instanceof Error ? err.message : "Error desconocido";
        setFiles((prev) =>
          prev.map((f) => (f.name === file.name ? { ...f, status: "error", error: message } : f))
        );
      }
    },
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-400 mt-1">
            Sube documentos PDF para que SchoolAI los use como contexto global al responder.
          </p>
        </div>

        <div className="flex gap-6">
          {/* Left column: dropzone + upload history + global docs list */}
          <div className="flex-1 space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
              <div className="px-5 py-4 flex items-center gap-2 border-b border-gray-50">
                ...
                <div>
                  <p className="text-sm font-semibold text-[#1a2b4a]">Fuentes globales indexadas</p>
                  <p className="text-xs text-gray-400">
                    Disponibles para todos los chats como contexto del agente
                  </p>
                </div>
              </div>
              ...
              {loadingDocs ? (
                ...
                <div className="text-center py-10">
                  ...
                  <p className="text-xs text-gray-400">No hay fuentes globales aún. ¡Sube el primero!</p>
                </div>
              ) : (
                  ...
                  {globalDocs.map((doc) => (
                    <div key={doc.id} className="px-5 py-3.5 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400">{doc.chunks_indexed} fragmentos indexados</p>
                      </div>
                      <span className="text-[10px] font-semibold bg-indigo-50 text-indigo-500 px-2 py-0.5 rounded-full shrink-0">
                        GLOBAL
                      </span>
                    </div>
                  ))}
            ...
            <div className="bg-[#1a2b4a] rounded-2xl p-5 text-white">
              ...
              <div className="mt-5 pt-4 border-t border-white/10">
                <p className="text-xs text-blue-200 leading-relaxed">
                  Los documentos se indexan automáticamente como fuentes globales y estarán
                  disponibles para que SchoolAI los consulte en cualquier chat.
                </p>
              </div>
            </div>
