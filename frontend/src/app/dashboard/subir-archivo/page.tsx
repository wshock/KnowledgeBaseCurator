...
export default function SubirArchivoPage() {
  ...
  const uploadFile = async (file: File) => {
    // Agrega el archivo al estado en "uploading"
    setFiles((prev) => [
      ...prev,
          ...
          <div className="w-64 shrink-0 space-y-4">
            <div className="bg-[#1a2b4a] rounded-2xl p-5 text-white">
              ...
              <div className="space-y-2">
                {[
                  { ext: "PDF", color: "bg-red-400", desc: "Documentos de texto" },
                ].map((item) => (
                  <div key={item.ext} className="flex items-center gap-3">
                    <span className={`${item.color} text-white text-[10px] font-bold px-2 py-0.5 rounded`}>
                      {item.ext}
                    </span>
              ...
              <div className="mt-5 pt-4 border-t border-white/10">
                <p className="text-xs text-blue-200 leading-relaxed">
                  Los documentos se indexan automáticamente y estarán disponibles para que SchoolAI los consulte al responder tus preguntas.
                </p>
              </div>
            </div>
